...
export class WeatherService {
  ...
  static async getCurrentWeather(
    ...
    console.log('🌍 [WeatherAPI] Fetching current weather...');
    ...
    try {
      ...
      console.log('✅ [WeatherAPI] Current weather data received:');
      ...
      console.log('🕐 Local time:', `${localTime.getUTCHours()}:${localTime.getUTCMinutes().toString().padStart(2, '0')}:${localTime.getUTCSeconds().toString().padStart(2, '0')}`);
      console.log('-----------------------------------');
  ...
  static async getForecast(
    ...
    console.log('📅 [WeatherAPI] Fetching 5-day forecast...');
    console.log('📍 City:', city);
    console.log('🌐 Language:', lang);
    ...
    try {
      ...
      console.log('📊 Total entries:', data.list.length);
      console.log('🗓️ First entry:', {
        date: data.list[0].dt_txt,
        temp: data.list[0].main.temp,
        condition: data.list[0].weather[0].description,
      });
      console.log('🗓️ Last entry:', {
        date: data.list[data.list.length - 1].dt_txt,
        temp: data.list[data.list.length - 1].main.temp,
        condition: data.list[data.list.length - 1].weather[0].description,
      });
      console.log('-----------------------------------');
