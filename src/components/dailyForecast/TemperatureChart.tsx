...
interface TemperatureChartProps {
  data: HourlyForecast[];
  gradientColors?: [string, string]; // [startColor, endColor] del gradiente de temperatura
}

export const TemperatureChart: React.FC<TemperatureChartProps> = ({ data, gradientColors }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <View className="items-center justify-center py-8">
        <Text style={{ color: "rgba(255, 255, 255, 0.6)" }}>{t('dailyForecast.noData')}</Text>
      </View>
    );
  }
  ...
  if (data.length < 2) {
    return (
      <View className="items-center justify-center py-8">
        <Text style={{ color: "rgba(255, 255, 255, 0.6)" }}>{t('dailyForecast.noData')} (insufficient data)</Text>
      </View>
    );
  }
